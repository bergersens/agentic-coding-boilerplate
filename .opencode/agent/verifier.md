...
actually holds and that the code is the *right* code, rather than trusting that
...
- **Test quality.** Right seam, public interface, non-tautological (expected
  ...
  refactor.
- **House rules.** Consistent with `AGENTS.md`, `CONTEXT.md`, and the project's
...
- **`FAIL`** — anything above fails. Return an ordered list of findings, each
  tagged, each stating *what* is wrong and *why* it matters:
...
- Never weaken or delete a test to reach green. If the *coder* did, that's a
  `[design]` must-fix.
